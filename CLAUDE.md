...
コードは P0〜P5 まで一通り main に入っている（2026-08-14）。残っているのは
**本番の設定と、実機・実耳での確認**で、どれも自分でしかできないもの。

### 本番を動かすのに必要（これが済むまで自動では何も走らない）

1. `supabase/scheduler.sql` を本番の Supabase で流す（pg_cron 未導入）。
   巡回・ワーカー・ダイジェスト・掃除の5本が登録される
2. Vercel の環境変数に `NEXT_PUBLIC_VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` /
   `VAPID_SUBJECT` を入れる（**値は手元の `.env.local` と同じものを**。
   別々に生成すると手元で登録した端末に本番からの通知が届かない）
3. 本番の Supabase に `0008`〜`0011` を流す（手元には適用済み。
   `npm run db:migrate` の接続先を本番にして実行）
4. 設定画面を一度保存して `settings` の行を作る（生成時刻・件数・指示文の確定）

### 自分でしか確かめられないこと

5. 通知をオンにして「テスト送信」が届くか（iOS はホーム画面に追加してから）
6. スマホ幅のレイアウトと、ホーム画面への追加（PWA）
7. **生成した音声を実際に聴いて品質を判断する**。声は
   `GEMINI_TTS_VOICE_A` / `_B` で差し替えられる
8. NotebookLM にダイジェストを入れて音声概要を鳴らす（P2 の本命の受け入れテスト）。
   出来上がりは向こうのほうが上なので、自前音声と聴き比べてどちらを主にするか決める

### 積み残しの検証

9. 複数スライドでの台本生成の再現（`gemini-3.5-flash` の枠が1日20回で、
   調査に使い切った日があった。枠が戻ってから `/exports` の
   「アプリ内で音声にする」で1本作る）

### 未実装

10. Google Drive への直接書き出し（OAuth）。plan.md の P2 で唯一残っているもの。
