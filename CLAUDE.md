...
- **重要度は二層にする**（2026-09-03 決定、`0036`）。AI が付ける
  `summaries.importance` は全ユーザー共通で、基準（新規性・影響範囲・一次情報・実務）は
  読み手が誰かを見ていない一般的なニュース価値でしかない。**「その人にとって重要か」は
  フォルダの重み（`folders.weight`、%）を後から掛けて表す。** フォルダは購読ごとの
  持ち物なのでユーザーごとになり、Gemini の呼び出しは1回も増えない。読み手ごとに
  要約を付け直す道は `0005` で潰れている（呼び出しが購読者数に比例する）。
  **効くのはダイジェストの選抜だけで、一覧の並べ替えには効かない**——一覧は
  `articles.importance` を DB 側で order して offset で継ぎ足しており、
  重みを掛けるとページの境目で記事が重複・欠落する。重み 0 は掛け算ではなく
  除外で扱う（掛けるだけだと枠の埋め戻しを通って結局載る）。
