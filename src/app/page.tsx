...
async function Reader({ searchParams }: PageProps<'/'>) {
  ...
  return (
    ...
    <div className="flex-1 flex min-h-0 overflow-hidden">
      ...
      {/*
        **一覧の HTML が端末に届いた時刻**を控える。

        実機（iPhone SE）の記録で、最初の描画が「応答を受け終わったあと」に
        しか来ていなかった（応答 12ms → 受け終わり 676ms → 描画 771ms）。
        これだけでは**サーバーが遅かった**のか、**届いていたのに描かなかった**
        のかが分けられない。ここは一覧の直後・本文ペインの `<Suspense>` より
        前なので、この時刻が小さければ「届いていたのに描かなかった」、
        大きければ「サーバーが遅かった」と決まる。
        読むのは components/BootTiming.tsx。

        インラインの `<script>` は、流れてきた HTML を parser が読んだ時点で
        走る（React が境界の中身を流し込むときも同じ）。**onLoad などに
